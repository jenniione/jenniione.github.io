...
| DEFINITION |
| ------ |
| 다음과 같은 공식을 카이제곱 통계략 혹은 피어슨 카이제곱 통계량이라고 한다. <br> <center>$$ \chi^2 = \sum_i \frac{(O_i - E_i)^2}{E_i} $$ </center><br> $$ \chi^2$$: 카이제곱 <br> $$O$$ : 관측값 <br >$$E$$ : 기댓값 |
...
그래서 이런 오차의 합이 나타나는 건 어려워 혹은 가능해라고 판단할 수 있는 <u>기준을 설정</u>할 것이다.  **5%(p-value)**도 드물다고 말해도 되는 굉장히 작은 확률이라고 생각한다. 그래서 관측치가 나타날 확률이 5%보다 적다면, 이건 일반적인 오차라고 보기 어렵다, 즉 내가 <u>관측한 오차에 뭔가 문제가 있다</u>고 판단할 것이다.
